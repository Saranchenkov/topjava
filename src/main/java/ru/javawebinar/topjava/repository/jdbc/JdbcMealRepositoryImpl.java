...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public List<Meal> getBetween(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        List<Meal> list = jdbcTemplate.query("SELECT * FROM meals WHERE user_id=? AND meals.datetime >= ? AND meals.datetime <= ? ORDER BY datetime DESC ", ROW_MAPPER, userId, startDateTime, endDateTime);
        return list.size() > 0 ? list : Collections.emptyList();
    }
}
